...
- **Ollama** running locally on port 11434 — `ollama pull nomic-embed-text`
...
The classifier uses an OpenAI-compatible endpoint at `http://localhost:3000` for intent classification and file summarization. Point this at whatever you're running — LiteLLM, an Ollama OpenAI-compat layer, or a hosted API.
...
4. Embeds each chunk via Ollama's `nomic-embed-text`
...
The MCP server reads three environment variables (all have sensible defaults):
...
All config lives in `codebrain.toml`. When running the ingestor inside Docker, `codebrain.docker.toml` is mounted instead — the only difference is the database URL uses the `postgres` service name and Ollama is reached via `host.docker.internal`.
...
[classifier]
model = "mistralai/mistral-small-2506"  # Model name forwarded to your proxy
base_url = "http://localhost:3000"       # OpenAI-compatible endpoint
...
1. Embeds your question with the same `nomic-embed-text` model
