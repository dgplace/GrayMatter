...
def _materialize_clusters_for_repo(
    cfg: dict,
    repo_name: str,
    embedder: EmbeddingClient,
    classifier: IntentClassifier,
    ...
    @param classifier Shared classifier client.
    @param no_classify Whether classifier calls are disabled.
    @return Tuple of `(cluster_count, granularity)` from cluster materialization.
    """
...
def _print_ingestion_summary(
    ...
    @param cluster_count Number of clusters materialized.
    @param cluster_granularity Granularity used by the cluster materialization pass.
    """
    console.print(f"\n[bold green]✓ Done[/]")
...
def main(
    ...
    try:
        ...
        cluster_count, cluster_granularity = _materialize_clusters_for_repo(
            cfg=cfg,
            repo_name=resolved_repo_name,
            embedder=embedder,
            classifier=classifier,
    ...
    _print_ingestion_summary(stats, cycle_count, cluster_count, cluster_granularity)
    _run_watch_mode(
        watch=watch,
        cfg=cfg,
        repo_root=repo_root,
